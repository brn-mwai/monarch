...
built on.
\item \textbf{Prediction.} The encoder, TRIBE v2 \cite{tribe2026}, returns activation over
$20{,}484$ fsaverage5 cortical surface vertices per time point.
...
\subsection{The instrument as published}

The apparatus is not described only in this chapter. It runs publicly at
\url{https://monarch-4iy.pages.dev}, and the corpus page there reads the same measurement
file this dissertation reports from, so the figures a reader sees on the page are the
figures in Chapter~5 rather than a summary written about them.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{assets/monarch-corpus.png}
\caption[The published corpus page]{The corpus page of the instrument at
\url{https://monarch-4iy.pages.dev/corpus/}, on 2 September 2026. The four values it reports
are the ones this dissertation reports: 400 items scanned, 331 of 400 with the ratio form
defined, a spread of $\Delta X = 0.1241$, and a range of $-0.070$ to $+0.054$. The page reads
\texttt{data/corpus.json}, exported from the same
\texttt{corpus\_naa.csv} the analysis scripts read, so the two cannot drift apart.}
\label{fig:site-corpus}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\textwidth]{assets/monarch-corpus-brain.png}
\caption[A per-vertex map on the published page]{One item's predicted response on the same
page, painted at every one of the $20{,}484$ vertices and thresholded at $+0.1043$, with the
two region means and their difference printed beneath it. The map is the encoder's stored
output for that item; the page renders no activation it did not scan, and shows an explicit
failure notice rather than a substitute when a prediction is unavailable.}
\label{fig:site-brain}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{../../services/inference/data/figures/B1_roi_definition.pdf}
\caption{The two ROI unions on the fsaverage5 surface: affective-salience in orange (1{,}030
vertices) and deliberative-control in blue (851). This figure is a definition and carries no
data.}
\label{fig:roi-definition}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Corpus composition. Each category is drawn from exactly one source dataset, which is the confound discussed throughout: category and provenance cannot be separated.}
\label{tab:corpus-design}
\end{table}
...
The sources are the ISOT fake and true news collections \cite{ahmed2017}, SemEval-2019
Task~4 \cite{kiesel2019}, Webis-Clickbait-17 \cite{potthast2018}, and PubMed abstracts
retrieved through the NCBI utilities \cite{pubmed}. The \texttt{manipulative} label is
derived from the source dataset rather than annotated per item.

NELA-GT-2021, named in the proposal, was withdrawn by its authors in January 2024. SemEval-2019
Task~4 is substituted; it is openly licensed, and the by-publisher collection used here labels
each article with the hyperpartisan rating of its publisher.
...
Output was verified before any analysis. The verification script,
\texttt{scripts/verify\_scan\_output.py}, checks every scanned row against the source corpus on text, category and identifier, requires
