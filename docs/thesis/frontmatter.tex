...
{\Large\bfseries Measuring the External Field:\\[6pt]
A Cortical-Proxy Content Observable and the\\[4pt]
Mean-Field Bound on Media-Driven Opinion Change}

\vspace{0.7cm}

{\large\itshape An instrument, a 400-item corpus,\\[3pt]
...
manipulative label at $\mathrm{AUC} = 0.6274$, against $0.9758$ for a TF-IDF baseline on the
same rows, so the index is reported as a field observable and not as a competitive classifier.
The separation is carried almost entirely by fear-activating content, $d = 0.9389$ against
neutral, while high-outrage content does not separate at all, $d = 0.0300$, though it is the
category the proposal expected to separate most; splitting the index into the two activations
it differences shows high outrage raising both by almost the same amount, $d = 0.507$ and
$d = 0.491$, so its flat difference is a symmetric response rather than an absent one. Each category is drawn from a single source dataset, so the separation is between
...
while $12.8\%$ of individual items reverse direction between sessions, a rate the measured
session noise predicts on its own, so the instrument supports category-level claims and no
per-item verdict is made.
...
Three papers were written from this project, and co-authorship with the supervisor is under discussion. Their
...
observable must satisfy to drive a transition.} To be submitted to the \emph{International Journal of Modern Physics C}.
...
a 400-item corpus, and the field bound it yields.} To be submitted to the \emph{Journal of Computational Social Science}, as
...
noise ceiling and a replication protocol.} To be submitted to \emph{Neurons, Behavior, Data analysis, and Theory}.
Reports the inter-subject noise ceiling measured from public data, and the two silent defects
behind a value this project withdrew. \emph{Status: drafted; the prediction pass over
the first 120~s is reported, and the full-episode pass remains.}
\end{enumerate}
...
runs at \url{https://monarch-4iy.pages.dev}.
