...
X$, no media-driven transition across the spinodal of the all-to-all mean-field model is
possible unless
\begin{equation}
...
($d = +0.030$, $p = 0.832$). Split into the two activations the index differences, high
outrage raises both against neutral by nearly the same amount ($d = +0.507$ and $d = +0.491$,
both $p < 10^{-3}$), so what looks like an absent response is a symmetric one that a
difference cannot see.

A TF-IDF baseline on the same rows and the same label reaches AUC $0.9758$. The index is not
the better classifier and is not offered as one; it is offered as the field observable
Chapter~3 requires.
...
\textbf{Not claimed.} That the index detects manipulation, or detects it better than
counting words does: it does not. That the separation is due to
...
The instrument itself runs at \url{https://monarch-4iy.pages.dev}. It carries the complete
...
X$ holds, within the all-to-all mean-field model, for every observable anyone proposes as a
field, and the burden it places on such
