...
cross-validation, and a sentiment score, the VADER compound valence, used directly as a score
...
sentiment score does not clear chance: its interval spans $0.5$ in both blocks and runs below
it on the restricted set. No claim that the index is a better detector of manipulative content
...
activations by almost the same amount ($d = +0.507$ and $d = +0.491$), so its null is a
