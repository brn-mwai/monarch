...
That ranking requires one normalisation to be meaningful. Since $h_c(\beta J)$ is fixed by the
population and does not depend on the observable, $\alpha_{\min} = h_c/\Delta X$ orders
candidates purely by $1/\Delta X$, and $\Delta X$ carries the units of $X$. Two observables
measured on different scales therefore cannot be compared as they stand: rescaling an
observable by a constant rescales $\alpha_{\min}$ by its inverse and can reorder the ranking
arbitrarily. The comparison is defined only once every candidate is expressed on a common
scale, and the natural choice is to standardise each observable by its own population standard
deviation before the spread is taken, so that $\Delta X$ is a spread in standard deviations and
$\alpha_{\min}$ is a required coupling per standard deviation of the observable. This thesis
evaluates one observable and so performs no ranking; the $\Delta X = 0.1241$ carried through
Chapter~5 is a raw spread in the encoder's standardised-activation units, which is $5.83$
standard deviations of the index across the corpus. Either scale is a legitimate report, and
the bound is unaffected as long as $\alpha$ is read in the matching units. What is not
legitimate is placing a second observable beside it without first putting both on the same
scale: that comparison is not a comparison but a statement about units.
